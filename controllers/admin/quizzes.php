...
class Quizzes extends MLMS_Controller {
	...
	{      
     ...
     public function ajaxexamtotask2($exam_id = NULL){
       $exam_id = ( $this->uri->segment(4) )  ? $this->uri->segment(4) : '' ;
  $courseNo = ( $this->uri->segment(5) )  ? $this->uri->segment(5) : '' ;
  $sectionNo = ( $this->uri->segment(6) )  ? $this->uri->segment(6) : '' ;
  $proid = ( $this->uri->segment(7) )  ? $this->uri->segment(7) : '' ;

  $copyExamid = $this->exam_model->DuplicateRecord('mlms_exam_paper', 'exam_id', $exam_id);
  $copyQues = $this->exam_model->DuplicateQues('mlms_exam_questions', $exam_id, $copyExamid['newID']);
  // print_r($copyExam);
  
  $questions = $this->exam_model->getexamdetail($copyExamid['newID']);
    if($copyExamid)
    {
      // echo $questions->exam_title;

      // print_r($questions);         

 switch ($questions->exam_type) {
          case 1:
            $qtype = 'Try and Learn';
            break;
          ...
          default:
            ...
            $quiz_secarr = $this->exam_model->get_count_sec($copyExamid['newID']); 
              $quiz_quesarr = $this->exam_model->get_count_ques($copyExamid['newID']);

       echo "<span class='col-sm-8'>".$copyExamid['title']."</span><div class='Qinfo'>
            <span><b>Marks: </b><div id='sel_mark'>".@$questions->total_marks."</div></span>
            <span><b>Quiz type: </b><div id='sel_type'>".@$qtype."</div></span>
            <span><b>contents: </b><div id='sel_details'>Section : ".$quiz_secarr."<br>Qustions : ".$quiz_quesarr->Qcount."</div></span>
          </div><span class='col-sm-4'> <a target='_blank' href='".base_url()."admin/exams/edit_exam/".$copyExamid['newID']." '>edit</a></span>
          <input id='db_media_1551' type='hidden' value='".$copyExamid['newID']."'>
          ";
     }
  }

public function ajaxDuplicate(){
    // $copyExam = $this->exam_model->DuplicateRecord('mlms_exam_paper', 'exam_id', '17');
  //   $copyExamid = $this->exam_model->DuplicateQues('mlms_exam_questions', '13', '18');
  // print_r($copyExamid);
  // print_r($copyExam);
}
    ...
    $this->template->set("mediaid",$id);
    $this->template->set("students",$students);
		$this->template->set_layout('backend');
		$this->template->build('admin/quizzes/quizstudents');
	}
	
	public function quizesaddtotask($courseNo, $sectionNo, $proid)
	{ 	$this->template->title('Exam add to Layout');
        $this->template->set('updType', 'create'); 
        $this->template->set('courseNo', $courseNo); 
        $this->template->set('sectionNo', $sectionNo); 
        ...
        else
        {
            ...
            $this->template->set("quizzes", $this->exam_model->getexamlist());
// print_r($this->exam_model->getexamlist());
	        //	}else{
	        //	$this->template->set("quizzes", $this->quizzes_model->getFinalQuiz($parent_id));
	        //	}
        }
