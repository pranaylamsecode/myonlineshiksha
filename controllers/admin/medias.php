	...
	?>
		...
		if($media->type == "docs"){
		 ...
		 else
		 {
			...
			// echo"<iframe src='".$pdffile."' width='100%' height='500px' frameborder='0' disableprint='true' style ='background:white'>myDocument</iframe>";
			//new code end 
...
<?php	}
   ...
   <!--	<script type="text/javascript" src="<?php echo base_url();?>public/jwplayer/vimeojwplayer.js"></script>  --><script src="<?php echo base_url();?>public/js/jquery-1.7.1.min.js" type="text/javascript"></script>
	...
	public function index($parent_id = NULL)
	{
      ...
      $start = ( $this->uri->segment(3))  ? $this->uri->segment(3) : 0;
       ...
       $this->template->set("medias", $this->medias_model->getItems_new($parent_id,$config['per_page'],$start,$search_string,$search_cat,$search_type));
   ...
   public function ajaxaddmedia($medlay = NULL,$medid = NULL)
    ...
    public function ajaxaddmodulemedia($medlay = NULL,$medid = NULL)
	{
      ...
      //$this->template->set("medias", $this->medias_model->getItems($parent_id,$config['per_page'],$start,$search_string,$search_cat,$search_type));
	   ...
	   $this->template->set("medias", $this->medias_model->getajaxMedia_new($parent_id,$term,$config['per_page'],$start,$search_string,$search_cat,$search_type));
      //print_r($this->medias_model->getajaxMedia_new($parent_id,$term,$config['per_page'],$start,$search_string,$search_cat,$search_type));
      //exit('anisha');
	   $this->template->set('categories',$this->medias_model->get_formatted_combo($parent_id));
	   $this->template->set('medfldgroup',$medlay);
	   $this->template->set('medfldid',$medid);
    ...
    $parent_id = NULL;
     ...
     // $pid = ( $this->uri->segment(4) )  ? $this->uri->segment(4) : 0;
	  ...
	  // $this->template->set("medias", $this->medias_model->getMedia($parent_id,$term,$config['per_page'],$start,$search_string,$search_type));
	   $this->template->set("medias", $this->medias_model->getMediaNew($parent_id,$search_string,$search_type));
	...
	function create($parent_id = FALSE)
	{
	  ...
	  if(($u_data['groupid']=='4'))
	  {
	   ...
	   //	$this->_set_rules();
		...
		//$form_data_aux			= array();
		$this->template->title("Create Course Media");
		$this->template->set('title', 'Create Media');
		$this->template->set('updType', 'create');
		$this->template->set('parent_id',$parent_id);
		//$this->template->set('media',(object) array());
            ...
            case 'image':
			     if($_FILES['file_i']['name'])
						...
						if($image_param['status'] == 'error')
					   {
							...
							//load the view and the layout
							$this->template->build('admin/medias/createsectionmedia');
  ...
  //           'created_by' => $user_id,
    ...
    {    
		...
		$whitelist = array('rar', 'zip', 'doc', 'docx', 'ppt', 'pptx', 'pdf', 'txt', 'jpg', 'png', 'gif', 'bmp');
    ...
    function createexercisefile2($newfilename)
	{
	    ...
	    if(($u_data['groupid']=='4'))
		{
			...
			$inserted_id = $this->medias_model->insertItems($dataNew);
	...
	function createexercisefile_old($parent_id = FALSE)
	{
		...
		$this->form_validation->set_rules('type', 'type', 'required');
		$this->form_validation->set_rules('name', 'name', 'required');
		//$this->form_validation->set_rules('file_f', 'file', 'required');
		//$this->form_validation->set_rules('description', 'description', 'required');
		$this->form_validation->set_rules('category_id', 'category', 'required');
		...
		$dataNew = array(
			...
			<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js"></script>
					<script type="text/javascript">
							...
							img_type = '<img src="'+img_path+'" alt="doc type1"/>';
	...
	function edit($id = FALSE, $parent_id = FALSE) 
	{
		...
		$this->template->title("Edit Course Media");
		...
		else
		{
		   ...
		   //	$imagename = ($form_data_aux['image']) ? $form_data_aux['image'] : $data['medias']->$index;
			...
			case 'file':
			     if($_FILES['file_f']['name'])
					{
						$file_param = $this->upload_file_flash();
...
//$file_info[0]->filename;
	...
	function delete()
	{
	    ...
	    if($mediarel_id)
	    {
		    echo 'This media must be assigned to any of the course. You cannot delete it.';
	    }
	    else
	    {
			$id = ($id != 0) ? filter_var($id, FILTER_VALIDATE_INT) : NULL;
	...
	//print_r($activation);
		...
		if ($activation) 
		{
            ...
            $this->session->set_flashdata('message', array( 'type' => 'success', 'text' => 'Media deactivated successfully!' ));
            }
		}
		else
		{
	...
	public function upload_file_i()
	{
	  ...
	  // echo json_encode(array('status' => $status, 'msg' => $msg, 'ftpfilearray' => $ftpfiles_i));
	   ...
	   return array('status' => $status, 'msg' => $msg, 'ftpfilearray' => $data['file_name'],'filename' => $data['file_name']);
	}
	...
	public function upload_file_f()
	{
	   ...
	   if ($status != "error")
	   {
		  $config['upload_path'] = getcwd().'/public/uploads/files';
		  $config['allowed_types'] = 'jpg|png|gif|bmp|rar|zip|doc|docx|ppt|pptx|pdf|txt|swf|mp3';
		  /* exe, zip*/
		  $config['max_size']  = 1024 * 100;
		  $config['encrypt_name'] = TRUE;
		  $config['overwrite'] = TRUE;
	...
	public function upload_file_flash()
	{
		$date = date('d');
  		$month = date('m');
  		$year = date('Y');
	...
	public function upload_exerfile_f()
	{
	   ...
	   if ($status != "error")
	   {
		  ...
		  //$config['allowed_types'] = 'pdf|jpg|doc|docx|txt';
   ...
   public function url_checking($str){
	...
	{
		$this->template->build('admin/medias/cropmediaimgsec');
	}
	...
	{
		$this->template->build('admin/medias/create_exercise_cat');
	}
	function saveexecategory()
	{
     ...
     //  $sessionarray = $this->session->userdata('logged_in');		
      ...
      //echo $program_name;
		...
		$this->medias_model->insertExeCategory($data);
		print_r($data);
	...
	$activation=$this->medias_model->activationItem($pid,$action);
		if ($activation) 
		{
            if($action==1)
            {
				echo 'Media activated successfully!';
            }
            else
            {
            	echo 'Media deactivated successfully!';
            }
		}
		else
		{
			echo 'Media activation fail!';
		}
	}
	...
	public function getallvideos(){
  	 $url='https://vimeoapi2.createonlineacademy.com/vendor/example/getvideos.php';
		$data= array('project_id'=>'628460','page'=>1); // Add parameters in key value
		...
		foreach ($data1 as $key) {
			$getlink = explode("/",$key->link);
			$con = "l.lecture_video = '".$getlink[3]."'";
			$getassign = $this->medias_model->getlect_assign($con);
			if (!empty($getassign))
				$assign_name = ucwords($getassign->name);
			else
				$assign_name = 'N/A';

			$output .= '<tr class="camp'.$i.'">
        <td class="field-title" style="text-transform: capitalize;color: #666;">
        '.$i.'
        </td>
        <td class="field-title" style="text-transform: capitalize;color: #666;">
          <img src="'.$key->image.'" width="110px" height="110px" alt="'.$key->alt_title.'">
        </td>
        <td class="field-title" style="text-transform: capitalize;color: #666;">
          '.$key->media_title.'
        </td>
