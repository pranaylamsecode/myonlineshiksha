...
class Promocodes extends MLMS_Controller {
  ...
  function create()
	{
	   ...
	   {
        if(empty($this->input->post('codelimit')))
          $codeused = "-1";
        else
          $codeused = 0;
        ...
        $data = array(
            ...
            'coupon_course' => $this->input->post('coupon_course'),
            'codeused' => $codeused
		    );
  ...
  function delete()
    ...
    $id = $this->input->post('id');
		...
		if ($isdelete)
		{
			echo "Promocodes deleted successfully";
		}
		else
		{
  ...
  public function changeStatus()
  {
    ...
    if($published==1)
    {
      echo 'Promo-code published successfully!';
    }
    else
    {
      echo 'Promo-code unpublished successfully!';
    }
  }
  ...
  {
      $code = $this->input->post('code');
      $getdata = $this->promocode_model->get_code($code);
      if (!empty($getdata)) {
          echo "1";
      }
      else
