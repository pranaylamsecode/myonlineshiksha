...
public function createPage($parent_id = NULL)
{
	...
	{
	    $session = $this->session->userdata('loggedin');
	    if(isset($_FILES['wed_image']))
                 {
	                 $file_name = $_FILES['wed_image']['name'];
	                 $file_size =$_FILES['wed_image']['size'];
	                 $file_tmp =$_FILES['wed_image']['tmp_name'];
	                 $file_type=$_FILES['wed_image']['type'];
	                 $file_ext=strtolower(end(explode('.',$_FILES['wed_image']['name'])));
	                  ...
	                  //   echo "Success";
	                 	...
	                 	if(!is_dir($path))
	                 	{
	                 		...
	                 		//exit('dir');
							  		...
							  		{
								  		do{
								  			$file_name = $path_parts['filename'].'_'.$i.'.'.end($temp);
								  			$i++;
	                   ...
	                   // echo 'assignment/'.$year.'/'.$file_name; unset($file_name);
                 	...
                 	$file_name = $_POST['widgets_img'] ? $_POST['widgets_img'] : '';
                 }
        $data = array(
		'title' => $title,
    	//'alias' => $alias,
        ...
        // redirect('admin/widgets/');
        		redirect('http://ideal.createonlineacademy.com/admin/templates/editoptions/45');
...
public function editPage()
{
	...
	{
		// print_r($_FILES['wed_image']);
		// echo $_POST['widgets_img']; exit('hh');
		$errors= array();
                 if(isset($_FILES['wed_image']['name']) && $_FILES['wed_image']['name'] != '')
                 {
	                 $file_name = $_FILES['wed_image']['name'];
	                 $file_size =$_FILES['wed_image']['size'];
	                 $file_tmp =$_FILES['wed_image']['tmp_name'];
