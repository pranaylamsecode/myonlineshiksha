...
class lcategories extends MLMS_Controller {
	...
	function __construct()
	{
		...
		$this->authenticate();
		error_reporting(0);
		$this->load->model('admin/lcategories_model');
     ...
     public function index($parent_id = NULL)
	{
      	if(isset($_POST['checkval']) && $_POST['checkval']){
	      	$checkval = $_POST['checkval'];
	       	$order = $_POST['order'];
	      	$res = $this->lcategories_model->orderdown($checkval,$order);
	      	if($res){
			   	echo $msg = "success";		}
			else{
				echo $msg ="error";
        ...
        $data = array(
       	...
       	$this->lcategories_model->updateOrder($cid[$i],$data);
		}
      }
	  $this->template->set_layout('backend');
	  $parent_id = NULL;
      ...
      $search_string = ($this->input->post('search_text', TRUE)) ? $this->input->post('search_text', TRUE) : $sess_pcat['searchterm'];
       ...
       $config['total_rows'] = $this->lcategories_model->getpcatcount($search_string);
       ...
       $this->template->set("categories", $this->lcategories_model->getItems($parent_id,$config['per_page'],$start,$search_string));
	   $this->template->set("countpcat", $this->lcategories_model->getcountpcat());
	   $this->template->append_metadata("<script src='js/sortertables.js' type='text/javascript'></script>");
	   $this->template->build('admin/lcategories/list');
	}
	...
	function create($parent_id = FALSE)
	{
		...
		{*/
			$this->template->build('admin/lcategories/create');
		/*}
		else
		{
			...
			$data5 = json_decode($uploadName,true);  
      		$imagename = $data5['ftpfilearray'] ? $data5['ftpfilearray'] : '';
		   	...
		   	$maxrow = $this->lcategories_model->maxRow($parent_id);
           	$orderingval = 0;
		    ...
		    $title = strip_tags($this->input->post('name'));
                ...
                $title2 = $titleURL;
                $avail_slug = $this->lcategories_model->match_slug($titleURL,$id);
                $i = 0;
           	...
           	$this->lcategories_model->insertItems($data);
	...
	public function create_action()
	{
		...
		if( $_FILES['imgname']['error']=='0' )
        {
			...
			{
				unlink('public/uploads/lcategories/img/'.$getcat->image);
			}
            $file_element_name = 'imgname';
            $config['upload_path'] = getcwd().'/public/uploads/lcategories/img/';
    ...
    public function upload_image()
	{
      ...
      $generate = $random_no.'_'.$year.'-'.$month.'-'.$date;
	   ...
	   if ($status != "error")
	   {
		  $config['upload_path'] = 'public/uploads/lcategories/img';
		  ...
		  else
		  {
			...
			$file_id = true;
			 if($file_id)
			 {
        		...
        		$config['source_image'] = FCPATH.'public/uploads/lcategories/img/'.$data['file_name'];
        		$config['new_image'] = FCPATH.'public/uploads/lcategories/img/thumb_232_216/'.$data['file_name'];
	...
	function edit($id = FALSE, $parent_id = FALSE)
	{
		...
		$this->template->set('category', $this->lcategories_model->getItems($id));
		...
		{		
			...
			$isupdated=$this->lcategories_model->updateItem($id,$form_data);
	...
	public function update_action()
	{
		...
		if( $_FILES['imgname']['error']=='0' )
        {
        	if(!empty($old_image))
			{
				unlink('public/uploads/lcategories/img/'.$old_image);
			}
            $file_element_name = 'imgname';
            $config['upload_path'] = getcwd().'/public/uploads/lcategories/img/';
            $config['allowed_types'] = 'gif|jpg|png|jpeg|GIF|JPG|JPEG|PNG';
	...
	{
		$id = $this->input->post('id');
		$pids = $this->lcategories_model->getProgramsByCatId($id);
		$pids2 = $this->lcategories_model->getProgramsByCatId2($id);
		if($pids && $pids2)
		{
		...
		else
		{
			...
			if ( $this->lcategories_model->getchildcount($id) )
			{
			...
			$isdelete=$this->lcategories_model->deletecat($id);
			if ($isdelete) 
			{
	...
	{
		$this->load->view('admin/lcategories/cropcategoryimg');
	}
    ...
    {	   
		...
		$file2 = FCPATH.'public/uploads/lcategories/img/'.$img_name;
		if(file_put_contents($file2, $data))//for upload file to the server
		{
                ...
                $config = array();
        		$config['image_library'] = 'gd2';
        		$config['source_image'] = FCPATH.'public/uploads/lcategories/img/'.$img_name;
        		$config['new_image'] = FCPATH.'public/uploads/lcategories/img/thumb_232_216/'.$img_name;
        		$config['create_thumb'] = TRUE;
        		$config['maintain_ratio'] = FALSE;
        		$config['master_dim'] = 'width';
                $config['width'] = 290;
		...
		$form_data =  array(						
 		...
 		$this->lcategories_model->updatecropItem($course_id,$form_data);
 	   }
    }

    function createcategory()
	{
		$this->load->view('admin/lcategories/createcoursecategories');
	}

	function cropcategoryimgpop()
	{
		$this->load->view('admin/lcategories/cropcategoryimgpop');
	}
	...
	public function uploadcourseimgpop()
    {
		...
		$date = date('d');
  		...
  		$generate1 = $random_no.'_'.$month.'-'.$date.'-'.$year;
  		echo $generate1 . '.png';
		$file2 = FCPATH.'public/uploads/lcategories/img/'.$generate1.'.png';
		if(file_put_contents($file2, $data))//for upload file to the server
		{
				$status = "success";
          		$msg = "File successfully uploaded";
                $config = array();
        		$config['image_library'] = 'gd2';
        		$config['source_image'] = FCPATH.'public/uploads/lcategories/img/'.$generate1.'.png';
        		$config['new_image'] = FCPATH.'public/uploads/lcategories/img/thumb_232_216/'.$generate1.'.png';
        		$config['create_thumb'] = TRUE;
        		$config['maintain_ratio'] = FALSE;
        		$config['master_dim'] = 'width';
                $config['width'] = 290;
    ...
    function savecategory()
	{
	    ...
	    if(($u_data['groupid']=='4'))			
	    {
        	...
        	$rs = $this->lcategories_model->insertItemspop($data);
	   		echo $rs;
	 	}
	}
	 ...
	 function assigncategory()
	 {
	 	  $newId =$_POST['newId'];
	 	  $oldId =$_POST['oldId'];	 	 
	 	  $rs = $this->lcategories_model->updateAssignCategory($oldId,$newId);
	 	  echo $rs;
	 }
	 ...
	 public function changeStatus()
	{
		...
		$in_ids = $qid;
		$publish=$this->lcategories_model->publish_unpublishItem($in_ids,$upd_data);
		if($published==1)
		{
	...
	public function check_dup()
	{
		...
		$get_res = $this->lcategories_model->match_slug($slug,$id);
		if(!empty($get_res))
		{
			print_r("0");
		}
