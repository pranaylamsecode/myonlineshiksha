 ...
 function authenticate()
  ...
  public function destradmin() 
   {     
	...
	{  
      ...
      // else $u_data = $this->session->userdata('logged_in');
      $userid = NULL;
      if($u_data['groupid'] != 4)
      {
        $userid = $u_data['id'];
  ...
  //new code end here  
    ...
    public function viewusers()
    {
      $this->load->model('admin/days_model');
       ...
       $this->template->set("days", $this->days_model->getlistDays($courseid));
  ...
  {    
        ...
        if($finalexamid)
        {
            ...
            $coursequizinfo2=$this->programs_model->getExamReport($userid,$progid,$finalexamid);
       ...
       if($certterm == 6)
     {
       ...
       $this->template->set('buydate', $this->programs_model->datebuynow($progid, $userid)->buydate);
       $this->load->model('admin/days_model');
       $this->template->set("days", $this->days_model->getlistDays($progid));
   ...
   public function view_attempt($examid,$stud_id,$pro_id){
     $this->template->set_layout('backend');

     $examid = $this->uri->segment(4) ? $this->uri->segment(4) : NULL;  //$examid;  
    $stud_id = $this->uri->segment(5) ? $this->uri->segment(5) : NULL;  //$stud_id; 
    $pro_id =  $this->uri->segment(6) ? $this->uri->segment(6) : NULL;   //$attempt; 
    $attempt =  $this->uri->segment(7) ? $this->uri->segment(7) : NULL;   //$pro_id;
 // $examid = '14';  
 //     $stud_id = '3'; 
 //     $attempt = '3'; 
 //     $pro_id = '182';
    ...
    $obt_marks = $this->exam_model->getmarks($examid,$stud_id,$pro_id,$attempt);
     ...
     $this->template->set('examinfo', $exam_detail);

      // $this->template->set("quizdetail", $this->Userreport_model->getQuizTakenById($qtid));


      $this->template->build('admin/studreport/view_attempt');
     ...
     if($this->Studreport_model->insertCertiData($dataarr))
     {
      ...
      //new code for email end
       // redirect('admin/studreport/viewusers/'.$pid);
          $issued = $this->programs_model->getUserName($issuedby);
           $data = array('approved' => 'yes', 'issuedby' => $issued, 'today' => $today);
...
function updateResultStatus()
 ...
 $data['fromemail'] = $urldomain;
  ...
  public function view_assignment($user_id,$assign_id){
      $this->template->set_layout('backend');
      $assignment_log = $this->Crud_model->GetData('mlms_assignment_log',"id,user_id, assign_id, que_id, answer, ans_attach, attempts, modified,remark,remark_attach","user_id = ".$user_id." and assign_id = ".$assign_id);
      $this->template->set('assignment_log', $assignment_log);
      $this->template->build('admin/studreport/assign_report');
  }
  ...
  public function send_remark(){
      ...
      $id = $this->input->post('id');
      $con = "id = ".$id;
      $getans = $this->Crud_model->get_single('mlms_assignment_log',$con,'remark,remark_attach');
      $remark_attach = '';
      if(!empty($_FILES['remark_attach'])){
        if( $_FILES['remark_attach']['error']=='0' )
        {
          if(!empty($getans))
          {
            if(!empty($getans->remark_attach) && !file_exists("/public/uploads/assignments/remarks/".$getans->remark_attach))
            {
              unlink(getcwd().'/public/uploads/assignments/remarks/'.$getans->remark_attach);
            }
          }
          $file_element_name = 'remark_attach';
          $config['upload_path'] = getcwd().'/public/uploads/assignments/remarks/';
          $config['allowed_types'] = 'jpg|JPG|png|PNG|jpeg|JPEG|gif|GIF|doc|docx|pdf|ppt|xls|xlsx|txt|mp4|mp3|3gp|avi|m4p';
          ...
          $remark_attach = $instr_file['file_name'];   
        }
        else if(!empty($getans))
        {
          if(!empty($getans->remark_attach))
          {
            $remark_attach = $getans->remark_attach;
          }
        }
      }else{
          if(!empty($getans->remark_attach))
