 ...
 //       $plans = $this->input->post('plan'); 
	...
	function edit_course($id = FALSE, $parent_id = FALSE)
	{
        ...
        //@@@@@@@@@ new code for exersice file end here 
			...
			$planforcond = $this->input->post('plan');
			$is_plans = 'free';
			if($planforcond == 'subscription')
				$is_plans = 'subscription';
			else if($planforcond == 'fixed')
				$is_plans = 'paid';

			$plandata = array(
						'is_plans' => $is_plans
			);
			$this->Crud_model->SaveData('mlms_program',$plandata,"id = ".$id);
  ...
  //       if(isset($get_reqid) && $get_reqid != '')
   ...
   function cropcourseimg($img)
	...
	public function get_batches($pro_id = ''){
		$program = $this->Crud_model->get_single('mlms_program',"id = ".$pro_id,"id,name,slug");
	    $batches = $this->Crud_model->GetData('mlms_batches','',"is_delete = 'no' and course_id = ".$pro_id);
	    $data = array(
	    			'program' => $program,
	    			'batches'	=> $batches,
	    );
	    $this->template->set_layout('backend');
		$this->template->build('admin/programs/batches_list',$data);
	}
	...
	public function action_create(){
		...
		redirect('admin/programs/get_batches/'.$course_id);
	}

	public function get_attendees($id = null){
		$auth = $this->session->userdata('logged_in');		
		if(!empty($auth)){
			$con = "za.event_id = ".$id;
			$events = $this->programs_model->get_all_attendees($con);
			$event = $this->Crud_model->get_single('zoom_meeting_list',"id = ".$id);
			$data = array(
						'events'		=> $events,
						'topic'			=> $event->topic,
						'lect_date'	=> date('Y-m-d h:i A',strtotime($event->start_time))
			);
			$this->template->set_layout('backend');
			$this->template->build('conference/attendees_list',$data);
		}
		else
		{
			redirect(base_url());
