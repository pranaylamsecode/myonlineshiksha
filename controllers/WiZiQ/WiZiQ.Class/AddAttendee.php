...
class AddAttendee
{
	...
	function AddAttendee($secretAcessKey,$access_key,$webServiceUrl,$webinarobj,$sessionarray,$dbname,$db_uname,$username)
	{
		...
		{	
 		...
 		if(!empty($XMLReturn))
 		{
			...
			if($attribNode=="ok")
			{
				...
				$attendeeid = $sessionarray['id']."^".$attendee_id.","; // 57^http://live.wiziq.com/aliveext/LoginToSession.aspx?SessionCode=dNQdmUIeDm%2bsObnodtqkSGMSvo4r8MEeghfd
				$attendeeid_field = $webinarobj->wiziq_attendee_id.$attendeeid;
				...
				{		
					...
					if($row_id['id'])
					{					   
					   $sql_Select = "SELECT `wiziq_attendee_url`,`wiziq_attendee_id` FROM `mlms_webinars` WHERE wiziq_class_id = '".$webinarobj->wiziq_class_id."' " ;
						  ...
						  foreach($url_field as $url_value)
						  {
						   ...
						   for($i=0;$i<count($userid_array);$i++)
						   {
							...
							if(empty($perfect_url))
							{
							   ...
							   $sql = "UPDATE `mlms_webinars` set wiziq_attendee_url='".$attendeeurl_field."',wiziq_attendee_id='".$attendeeid_field."' WHERE wiziq_class_id = '".$webinarobj->wiziq_class_id."' " ;
			...
			else if($attribNode=="fail")
			{
				...
				//echo "<script>document.location.href='conwebinar/?msg=".$errorcode."'</script>";
