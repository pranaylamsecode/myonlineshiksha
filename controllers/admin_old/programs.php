...
class Programs extends MLMS_Controller {
    ...
    { 
      ...
      {
       redirect('admin/users/login','refresh',301);
      }      
    }
    ...
    public function index($parent_id = NULL)
	{
       ...
       $u_data=$this->session->userdata('loggedin');
         ...
         $progassign=$this->programs_model->getItems($parent_id,$config['per_page'],$start,$search_string,$search_status,$search_cate);
       /*else
	   ...
	   $this->template->set("countprog", $this->programs_model->getcountprogram());
	 ...
	 public function reviews($parent_id = NULL)
	{
	...
	{	 
		...
		//$this->template->set('teachers',$this->programs_model->getAssistant());
		...
		else
		{
			...
			$data5 = json_decode($uploadName,true);  
      		$imagename = $data5['ftpfilearray'] ? $data5['ftpfilearray'] : 'no_images.jpg';
         ...
         if($this->input->post('avg_cert')){
          ...
          $this->programs_model->updateCertificates($upd_data);
			...
			if((!empty($_POST['save2'])) && ($_POST['save2']=='Save & Back to list'))
			   	...
			   	//redirect('admin/days/'.$inserted_id);
			   	redirect('admin/programs/'.$parent_id);
			   ...
			   if((!empty($_POST['submit'])) && ($_POST['submit']=='Save Changes'))
			   {
			   	...
			   	redirect('admin/section-management/'.$inserted_id);
			   	//redirect('admin/programs/'.$parent_id);
			   	//redirect('admin/programs/edit/'.$inserted_id);
			   	//$this->load->view('admin/programs/create_lectr_pop.php');
...
<script src="<?php echo base_url(); ?>public/craftpip-jquery-confirm/dist/jquery-confirm.min.js" type="text/javascript"></script>
		...
		//var $i =jQuery.noConflict();		
			document.location.href = window.location.origin+'/admin/course-manager/';		
	...
	{
		$this->template->build('admin/programs/cropcourseimg');
	}
	...
	public function createweb()
	{
		...
		if ($this->form_validation->run() === FALSE)
		{
			$this->template->build('admin/programs/createwebinarcourse');
		}
		else
		{
			$alias = ($this->input->post('aliase'))?$this->input->post('aliase'):$this->input->post('name');
       ...
       // echo $inserted_id; print_r($data); exit("$inserted_id");
        $data2 = array(
        		'proid' => $inserted_id,
		...
		$inserted_webid = $this->programs_model->insertWebinar($data2);
 ...
 //       $plans = $this->input->post('plan'); 
          ...
          $this->programs_model->updateCertificates($upd_data);
			...
			if((!empty($_POST['save2'])) && ($_POST['save2']=='Save & Back to list'))
			   ...
			   if((!empty($_POST['submit'])) && ($_POST['submit']=='Save Changes'))
			   {
			   	...
			   	redirect('admin/programs/createweb/'.$inserted_id);
			   	//redirect('admin/programs/'.$parent_id);
			   	//redirect('admin/programs/edit/'.$inserted_id);
			   	//$this->load->view('admin/programs/create_lectr_pop.php');
	...
	{	   
		...
		// print_r($this->programs_model->getWebinar($parent_id,$id)); exit("controller");
		$this->template->set('updType', 'edit');
		$this->template->set('id', $id);
		$this->template->set('user_id', $user_id);
		$this->template->set('parent_id', $parent_id);
       ...
       // }
        ...
        //@@@@@@@@@ new code for exersice file end here 
			...
			$updateWeb = $this->programs_model->updateWebinar($webid,$data2);
  ...
  //       if(isset($get_reqid) && $get_reqid != '')
		...
		if ($isupdated) // the information has therefore been successfully saved in the db
		{
			...
			foreach ($files_to_delete as $index)
			{
			   ...
			   {
			   	 redirect('admin/programs/editwebinar/'.$id);
