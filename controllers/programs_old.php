  ...
  //      return;	  
	...
	{	
		...
		$checkuser = $this->program_model->getBuyCoursesUser($user_id,$course_id);
		 ...
		 $data['quizcomment'] = $this->program_model->getLessonQuery($course_id);
...
function promoCodeApply()
{
    ...
    $applied = false;
   	...
   	{	
		...
		if($rs->coupon_for == 1)
		{
			$coup_courses = explode(',', $rs->coupon_course);

			 if(in_array('"'.$course_id.'"', $coup_courses))
			 	  $applied = true;
			 else $applied = false;			
		}
		else{ $applied = true; }
	    ...
	    $promoDateBegin = date('Y-m-d', strtotime($codeStart));	    
	    $promoDateEnd = date('Y-m-d', strtotime($codeEnd));
	    ...
	    if(($actulrate !='') && ($paymentDate >= $promoDateBegin) && ($paymentDate <= $promoDateEnd))
	    {
	      
	    	 if($codelimit != 0)
				{	

					if($codelimit > $codeused && $codelimit > 0)
					{			
