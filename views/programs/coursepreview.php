...
.header_sect {
   ...
   /* box-shadow: inset 0 0 10px #908F8F;*/
}
.main_head {
    width: 50%;
    margin: 9px auto;
...
<div class="main"> <!-- <a id="go-back" href="<?php echo base_url(); ?><?php echo $urlCourse;?>/lectures/<?php echo $pro_id; ?>" class="pos-r zi1 ml15 mt15 dif text-topaz fs12 bold ml0-md"> <i class="icon-chevron-sign-left fs16 mr5"></i> <span class="fs10-md w130 ellipsis-2lines">Back to Course</span> </a> -->
		...
		foreach ($days as $day)
                {
                	...
                	if($moduleid == $day->id)
                	{
                   		echo "<b style='color:ededed;'>".$day->title." :</b> ";
                    	...
                    	foreach ($lessons as $lessonn)
                    	{
                    		...
                    		{
                         	// echo "";
                         	 echo "<span style='color:bedbbd;'>".$lessonn->name."</span>";                
                            }     
                   		}
                   }
  ...
  <!--  <div id="theme_color" class="theme_color">
    	...
    	<b class="dropbtn"><i class="entypo-cancel"></i>Close Preview</b> -->
    ...
    <li class="on" data-lectureid="2335868">
		 <!-- <div class="prev-lecture" id="prev-lecture" style="display:block">
			...
			if($prevurl != NULL)
			{
				...
				{				
				?>
				<a href="javascript:void(0)" onClick="nextslide('<?php echo $pro_id;?>','<?php echo $mod_id;?>','<?php echo $prelesson_id;?>','1');" ><i class="icon-chevron-up"></i></a><span style="cursor:pointer" onClick="nextslide('<?php echo $pro_id;?>','<?php echo $mod_id;?>','<?php echo $prelesson_id;?>','<?php echo $lessonSrNo;?>','<?php echo $lesson->layoutid; ?>','<?php if($lesson->layoutid == '22'){ echo $lesson->is_webinar; } else if($lesson->layoutid == '2'){ echo $lesson->is_assignment; } ?>');" >Previous Lecture</span>
				<?php 
			    }
			}
       ...
       <div class="main_head"> 
           <div class="pre_head"> <b style="font-size: 16px;color: seashell;">Preview Mode </b><div style="color:white;"> This is how your lecture will be displayed to students</div></div>  
       </div>
         <div class="close_btn" style="" id="close_btn"> <a><i class="entypo-cancel"></i> Close Preview </a></div> 
  </div>
		...
		if($assign_check<='0')
		  ...
		  // echo json_encode($assignment);
		   ...
		   //echo json_encode($assignment);
				...
				echo "<p style='margin-top: 0;'><center><h2 style='color:#fff;text-align:center'>Assignment : ".$assignment['info']->assign_title."</h2></center></p>";
				// print_r($assignment['info']);
				echo "<input name='assign_idd' value='".$assign_id."'  style='display:none'>";
				if($assignment['info']->estimated_time){
				echo "<p style='margin-bottom: 0px; text-align:center;'><span style='color:#fff'>".$assignment['info']->estimated_time." to complete</span>";
				}
				echo "</div></div>";
				...
				echo "<div id='style-1' style='display:none' ><div id='instruct'><h2>Assignment Instructions</h2><br><br><div class='col-sm-offset-2' id='instructions'>".$assignment['info']->assign_instruction."</div>";
				if($assignment['info']->instruction_videos)
				{
				echo "<br><b>Instuction Video</b><br><div id='video11' ><center><video width='420' height='280' controls><source id='r_video' src='".base_url()."public/images/".$assignment['info']->instruction_videos."' type='video/mp4'></video></center><br></div>";	
				}
				...
				{		
    				...
    				$fileExtension = array('jpeg', 'jpg', 'png', 'gif', 'bmp');
         			...
         			{
         				echo "<br><b>Resource Media</b><br><center><img id='r_file' max-width='420' max-height='280' src='".base_url()."public/images/".$assignment['info']->resources_files."'></center><br>";
         			}
         			else{
         				
         				echo "<br><b>Resource Files</b><br><a style='width:250px; word-wrap:break-word;' href='".base_url()."lessons/filedownload/".$assignment['info']->resources_files."' onclick='return false' id='".$assignment['info']->resources_files."' ondblclick=filedownload(this) data-toggle='tooltip'  title='Double click to download' >".$assignment['info']->resources_files."</a>";
         			}	 
				}
				...
				{
					echo "<div id='Ques'><br><h4>Questions for this Assignment<br></h4></div>";
					$i = 1;
					foreach ($assignment['content'] as $key1 => $value1) 
					{
						echo "<div class='Qcon' id='Qcon_".$i."'>".$i.") ".$value1['que_text']."</div></br>";
				...
				echo "<div id='submission'><div class='assgnmt_grp_btn'></div><p><h2>Assignment Submission</h2><br>Save or Submit your work | <span id='status_ass' style='display:none' class='assgnmt_submit'> Assignment Submitted</span></p>";
				...
				{	
					...
					foreach ($assignment['content'] as $key1 => $value1) 
					{
						echo "<br><br><div class='Qset' id='Qset_".$i."'>".$i.". ".$value1['que_text']."<br></div>";
			            ...
			            echo "<input  name='stu_Att' onclick='submit_block()'  class='stu_Att' data-icon='true' id='stufilestyle_".$i."'  style='position: absolute; clip: rect(0px 0px 0px 0px);'></span>";
			            	// type='file' onchange='stuAnsfile(this)'
					  ...
					  // alert(  value1.assign_id );
						$i++;
																						...
																						// onclick='submit_confirm()'
				echo "<br><br><button type='button' class='subconfirm btn btn-success' onclick=submit_block('assign') style='float:right' >Submit</button>";
			        	...
			        	?>
					      ...
					      <?php
					      		  ...
					      		  // {
					      		  	?>
					      		  <!-- 	<div class="lllt_txt lesson_<?php echo $lessonSrNo;?>">
										<a onClick="javascript:void(0)" class='<?php echo (($not_show == false) && ($user_id > 0))?"fancybox fancybox.iframe":'';?>' > <?php// echo $lesson->name;?></a> 
...
<!--         //      <p>There are no available downloads.</p>
 -->
      ...
      if($exercise_count == 0)
      {
        echo "<p>There are no any lecture exercise available for downloads.</p>";
      }
       ?>
 ...
 <?php	
	...
	if($this->program_model->checkEnrolled($user_id,$program_id))
    {
		...
		foreach($webinars as $webinar)
		{
			...
			<input type="hidden" value="<?php echo $u_fname; ?>" name="ufname">
			...
			<div style="display:inline-block; width:100%; padding:0;"> <a style="float:left"><?php echo $webinar->title;?></a>
    ...
    <div id="notes" class="ud-notetaking" data-courseid="409734">
      ...
      <div style="clear:both;">
        <div id="notes-mask">
          ...
          <ul id="notes-list" >
		  <?php
		$data =array( 'pid' => $this->uri->segment(4),
			'module_id' => $this->uri->segment(5),
			'lesson_id' => $this->uri->segment(6)			 
		   );
...
<script>
	...
	{	
		// alert(lesson_id);alert(layoutid);
...
///////////////
		...
		{	
			...
			{	
				...
				var str = "<br><br><div class='panel panel-primary' style='height:auto' data-collapsed='0'><div class='panel-heading'><div class='panel-title' style='padding-bottom: 0px;'>";
				str += "<p style='margin-top: 0;'><center><h2 style='color:#fff;text-align:center'>Assignment : "+obj.info.assign_title+"</h2></center></p>";
				str += "<input name='assign_idd' value='"+web_id+"'  style='display:none'>";
				if(obj.info.estimated_time){
					
				str += "<p style='margin-bottom: 0px; text-align:center;'><span style='color:#fff'>"+obj.info.estimated_time+" to complete</span></p>";
				}
				str += "</div></div>";
				...
				str += "<div id='style-1' style='display:none' ><div id='instruct'><h2>Assignment Instructions</h2><br><br><div class='col-sm-offset-2' id='instructions'><br>"+obj.info.assign_instruction+"</div>";
				if(obj.info.instruction_videos)
				{
				str += "<br>Instuction Video<br><div id='video11' ><center><video width='420' height='280' controls><source id='r_video' src='<?php echo base_url() ?>public/images/"+obj.info.instruction_videos+"' type='video/mp4'></video></center><br></div>";	
				}
				...
				{		
    				...
    				var fileExtension = ['jpeg', 'jpg', 'png', 'gif', 'bmp'];
         			...
         			{
         				str += "<br><b>Resource Media</b><br><center><img id='r_file' max-width='420' max-height='280' src='<?php echo base_url() ?>public/images/"+obj.info.resources_files+"'></center><br>";
         			}
         			else{
				...
				if(key == 'content')
				{	str += "<div id='Ques'><br><h4>Questions for this Assignment<br></h4></div>";
					var i = 1;
					$.each( value, function(key1, value1 ) {

						str += "<li class='Qcon' id='Qcon_"+i+"'>"+i+") <div style='float:right'>"+value1.que_text+"</div></li>";
					  // alert(  value1.assign_id );
						i++;
						if(value1.que_attachment)
						{
				...
				str += "<div id='submission'><div class='assgnmt_grp_btn'></div><p><h2>Assignment Submission</h2><br>Save or Submit your work | <span id='status_ass' style='display:none' class='assgnmt_submit'> Assignment Submitted</span></p>";
				...
				{	str += "<div id='Ques'><br><h4>Questions for this Assignment<br></h4></div>";
					...
					$.each( value, function(key1, value1 ) {
						...
						str += "<br><br><div class='Qset' id='Qset_"+i+"'>"+i+". "+value1.que_text+"<br></div>";
			            ...
			            str += "<input  name='stu_Att' onclick='submit_block()'   class='stu_Att' data-icon='true' id='stufilestyle_"+i+"'  style='position: absolute; clip: rect(0px 0px 0px 0px);'></span>";
          															// onchange='stuAnsfile(this)'
       ...
       str += "<br><br><button type='button' class='subconfirm btn btn-success' onclick=submit_block('assign') style='float:right' >Submit</button></div>";
       																			// onclick='submit_confirm()'
...
function web_start()
{
	var msg = "Not allowed to attent webinar here...";
	submit_block(msg);
} 

function submit_block(msg)
    {
    	if(msg == "assign")
    		var strcontent1 = "You can not submit assignment here.";
    	else if(!msg)
    		var strcontent1 = "You are unable to perform any activity here.";
    	else
    	 var strcontent1 = msg;
      	
      $.alert({ 
      title: 'You are on preview mode!',    
      content: '<h4 style="padding:2%;text-align:center;font-weight:bold;color:#016ac1">'+strcontent1+'</h4>',
  });
    }

 function submit_confirm()
    {
      var strcontent1 ='<center><h4 style="padding:2%;text-align:center;font-weight:bold;color:#016ac1">You will not able to edit after you submit.</h4></center>';
    ...
    function submit_ans()
    {
      ...
      var str2 = "<div class='InstructorView' id='instrutorsubmitview' ><h4 class='instrt_ex_head'>Instructor Examples</h4><br><h4></h4><br>";
        var str = "<div class='InstructorView' id='yoursubmitview' ><br><b>Your Submission</b><br><br>";

     for (var i = 1; i <= count; i++) 
      {
      //$('.Ansview').css('display','none');
        var content = tinyMCE.get('ansview_'+i).getContent();

        var Instructcontent= $('#Q_'+i).find('#ansdiv_'+i).find('#Qanswer_'+i).html();

        $('#submission').find('#subview_'+i).text(content);
        ...
        var Qtext = $('#Qset_'+i).text();
            ...
            if(attvisible == true)
            {
             ...
             if(Instructcontent)
            {               
              str2 += "<div><b>Instructor Suggestion </b><br>"+Instructcontent+"</div><br>";
            ...
            {               
              ...
              var attvisible = $("#submission").find('#stufile_'+i).is(':visible');
                  if(attvisible == true)
                  {
                    var filename = $("#submission").find('#stu_ans_att_'+i).text();
                   var arr =  filename.substring(filename.lastIndexOf('.') + 1).toLowerCase();
                   var fileExtension = ['jpeg', 'jpg', 'png', 'gif', 'bmp'];
      ...
      $('#submission').find('.textview').css('display','none');
      
      $('#submission').find('.subview').css('display','block');

      $('#submission').find('.subconfirm').css('display','none');
...
{
	var msg = "you can not make mark as complete";
	submit_block(msg);
	//alert("you can not make mark as complete");
		// var pro = $("#proid").val();
		// var mod = $("#modid").val();
		// var les = $("#lessid").val();
		// //alert(pro);
...
{ 
	var msg = "You can not start exam here";
	submit_block(msg);
	//alert("You can not start exam in preview mode");
...
<script>
	 function convert_plyr(){
 	 $(document).find('.asset-container').find(".vplyr").each(function () {
		     // count++;
 	 ...
 	 $(document).find('iframe').attr('webkitAllowFullScreen', '');
 	 $(document).find('iframe').attr('mozallowfullscreen', '');
 }
  window.onload = function(){
   setTimeout(function() {
		convert_plyr();
