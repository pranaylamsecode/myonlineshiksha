...
<p class=""><i>Choose the type of the Question you with to create from the below tabs</i></p>
 ...
 -->
   ...
   /></textarea>
						...
						<div class="form-group">						
							...
							<div class="col-sm-12 no-padding">			
														<!-- name="txtPoints"  -->
	                      		<input type='text' id="txtPoints_reg" name="txtPoints_reg"  class="form-control tempinput point" onkeypress="return isNumberKey(event)"    />
 ...
 -->
	...
	<button type="button" id="nxt_que">Next Question</button>
	<button type="button" id="nxt_sect">Next Section</button>
	<button type="button" id="nxt_pg">Next Page</button>
...
url:  "<?php echo base_url(); ?>admin/admin/exams/autosave",
 // dataType: "json",
...
success: function(ins_id) {
	...
	if(res[1])
	{
		...
		var tempvalarray = res[0].split('_');
		var qid = $('#Qform').find('#Q_id').val();
		if(!qid){
			console.log(tempvalarray);
			tempvalarray = tem.split('_');
		$('#Que_'+tempvalarray[1]).find('div').find('#qid_'+tempvalarray[1]).val(res[1]);
		$('#Qform').find('#Q_id').val('');
...
setup : function(ed) {
      ...
      ed.on('KeyUp', function(ed, e) {
		...
		var tempvalarray = tempval.split('_');
  		...
  		// 	var test = $('#ques_'+tempvalarray[1]).text();
    // console.log(test);
  			  		//	$('#ques_'+tempvalarray[1]).html($(tempval2).text());
