...
//print_r($getTheme); 
  ...
  $meta_description2 = (isset($getmeta->meta_desc) && trim($getmeta->meta_desc)!= '') ? trim($getmeta->meta_desc) : "MLMS Institute"; 
	 ...
	 if($this->uri->segment(3)){
	  ...
	  $getmeta = $CI->settings_model->getMetaTag($pro_id); 
	  if($getProgram){
	   $cdesc= strip_tags($getProgram->description);
...
.widget_media img, .widget_media video {
  width: 100%;
}
.container.info .screenres .span4 .widget_media img {
...
?> 
	...
	<?php }else{
		if($this->uri->segment(1) == 'blog' && $this->uri->segment(3) != '' && is_numeric($this->uri->segment(3))) { 
			$CI->load->model('blogs_model');
			$blog = $CI->blogs_model->getBlog($this->uri->segment(3));
			?>
	...
	<meta name="generator" content="<?php echo $blog->meta_title; ?>" />
    ...
    <section class="extra">
        <div class="container">
            <div class="row-fluid">                
				...
				?>					
                <div class="span4 new-testimonial">
						 ...
						 <?php if($row->image)
					        {
					          ...
					          {
					         ?>
					          <img width='341' height='auto' src="<?php echo base_url() ?>public/uploads/widgets/<?php echo $row->image ?>">
					      <?php  }
					        else if(in_array(end($exploded), $video)){ ?>
					          <video class="myvideo_bottom" controlslist="nodownload" disablePictureInPicture width='341' height='auto'><source src="<?php echo base_url() ?>public/uploads/widgets/<?php echo $row->image ?>" type="video/mp4"></video>
					    <?php    }
					      } ?></div>
...
<script>
  $(document).find('.myvideo_bottom').hover(function () { if (this.hasAttribute("controls")) { //
