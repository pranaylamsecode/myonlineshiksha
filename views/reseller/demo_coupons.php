...
<div class="generate_coupon_btn_box">
 <!-- <button class="btn btn-primary btn-md pull-right generate_coupon_btn" type="button" data-toggle="collapse" data-target="#new_coupon" aria-expanded="false" aria-controls="collapseExample"> Generate Coupon Code <i class="fa fa-plus" aria-hidden="true"></i> </button> -->
 <strong style="font-size: 20px; color: #373e4a;">Generate New Coupon Code</strong>
 <button class="btn btn-success btn-md control-label" type="submit" style="font-size: 18px;height: auto;" <?php /* if(intval($total_unused)>=5){ echo 'disabled';} */?> > Generate Coupon </button>
 </div>
<div class="form-body" id="new_coupon">
   <label class="col-sm-12 col-xs-12 control-label field-title">Generate Coupon Code 
   <form method="post" action="<?php echo base_url(); ?>reseller_Settlements/create_coupon">
      <div class="row form-border">
         <div class="col-md-6 col-sm-12 col-xs-12">
            <label class="control-label field-title">Customer Name<span class="error">* </span>: </label><span class="error" id="err_name"></span>
            <input type="text" class="form-control field-title" name="name" id="name">
         </div>
         <div class="col-md-6 col-sm-12 col-xs-12">
            <label class="control-label field-title">Mobile No.<span class="error">* </span>: </label><span class="error" id="err_mobile"></span>
            <input type="text" class="form-control field-title" onkeypress="return only_number(event);" maxlength="12" name="mobile" id="mobile">
         </div>
         <div class="col-md-6 col-sm-12 col-xs-12">
            <label class="control-label field-title">Course Name <span class="error">* </span>: </label><span class="error" id="err_course_id"></span>
            <select class="form-control field-title" name="course_id" id="course_id">
               <option value="">select Course</option>
               <?php foreach ($courses as $key) { ?>
               <option value="<?php echo $key->id; ?>"><?php echo $key->name." - &nbsp;&#8377; ".$key->fixedrate; ?></option>
               <?php } ?> 
            </select>
         </div>
      </div>
      <div class="row form-border">
         <div class="col-md-4 col-sm-6 col-xs-12">
            <button class="btn btn-success btn-md control-label form-control" type="submit" onclick="return check_error()" style="font-size: 18px;height: auto;" <?php /* if(intval($total_unused)>=5){ echo 'disabled';} */?> > Generate Coupon </button>
         </div>
       </div>
    </form>
</div>
     ...
     <table class="table table-bordered table-striped datatable dataTable" id="table-2" aria-describedby="table-2_info">
        ...
        <tbody role="alert" aria-live="polite" aria-relevant="all" id="payout_content">
           ...
           <tr class="camp0">
              ...
              <td class="field-title" style="text-transform: capitalize; color: #949494; text-align:center !important;"><?php echo $key->student_name;?></td>
              <td class="field-title" style="text-transform: capitalize;color: #949494; text-align:center !important;"><?php echo $getcourse->name;?></td>
              <td class="field-title" style="text-transform: capitalize;color: #949494; text-align:center !important;"><i class='fa fa-inr'></i> <?php echo number_format($getcourse->fixedrate,2);?></td>
