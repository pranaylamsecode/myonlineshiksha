...
$('#addbut').live('click',function(e) {
       ...
       var ischecked = $("input:checked").length;   
           if (ischecked != toSelect){
			alert("Please select "+toSelect+" Courses.");
			return false;
		}
               ...
               $('.checkquizzes:checked').each(function() {
                ...
                if ($('#qli'+$(this).val(),parent.document).length > 0) {
                 ...
                 var qname = $("span#queezname"+$(this).val()).html();
                  ...
                  $("#coursestoaddlist",parent.document).append('<tr id="qli'+$(this).val()+'" style="cursor: move;" class="quizrow" ><td>'+$(this).val()+'</td><td>'+qname+'</td><td><span class="removespan"><a href="javascript:void(0);" onclick="deleteRow(this)" class="removeele" id="remove'+$(this).val()+'">Remove</a></span></td><td hidden="hidden"><td hidden="hidden"><input type="hidden" name="qidck[]" id="qidck" value="'+$(this).val()+'"></td></tr>');
...
<a style="list-style: none; float: right;">
	...
	<input type="hidden" name="toSelect" id="toSelect" value="<?php echo $no_of_courses; ?>">
</a>
</div>
</div>
<div class="clr"></div>
...
echo ($updType == 'create') ? form_open(base_url().'admin/templates/courselist', $attributes) : form_open(base_url().'admin/quizzes/editque/'.$question->id.'/'.$qid, $attributes);
?>
...
<?php foreach ($courses as $course):  
     ...
     $days = $CI->Programs_model->getlistDays($course->id);
     
     foreach ($days as $day)
		{
			$lessonsch = $CI->Programs_model->getLessons($day->id);
			if(empty($lessonsch))
			{
				continue;
			}
...
?>
	<tr class="camp<?php echo $i;?>"> 	    
		<td>
        <input type="checkbox" title="Checkbox for row <?php echo $i;?>" value="<?php echo $course->id?>" name="checkquizzes[]" id="checkquizzes[]" class="checkquizzes">
        </td>
	    <td><?php echo $course->id;?></td>
	   
	    <td><span id="queezname<?php echo $course->id?>"><?php echo $course->name;?></span></td>
  </tr>
<?php
 }
 unset($lessonshasvalue);
endforeach ?>
