...
<script type="text/javascript">
      ...
      var optionsCertificateBackground = { 
    //url: "<?php echo base_url(); ?>admin/certificates/upload_image/" ,
   // type: "post", 
    beforeSend: function() 
    {
        ...
        $("#submit").attr('value',"Wait,Uploading..");
        $("#progress_video").show();
        //clear everything
        $("#bar_video").width('0%');
        $("#message").html("");
    ...
    { 
      console.log(response);
        ...
        {
          $("#bar_video").width('100%');
          $("#percent_video").html('100%');
          $('#file_v').val('');
          $("#progress_video").hide();

          if(data.status != 'success')
             {
                    var str = "<p style='font-size: 20px;text-align: center;padding-top: 10px;'>"+data.status+"</p>";
                $.alert({
                    title: '',
                    content: str,
                });
...
$("#cform").ajaxForm(optionsCertificateBackground);
...
echo ($updType == 'save') ? form_open_multipart(base_url().'admin/certificates/upload_media/', $attributes) : form_open_multipart(base_url().'admin/certificates/upload_media/');
...
<input type="submit" name="uploadbtn" id="uploadbtn" value="Upload Image" style="display:none">
...
$attributes = array('class' => 'tform', 'id' => 'cform1');
