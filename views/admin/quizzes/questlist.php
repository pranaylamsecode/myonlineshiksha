...
<script src="<?php echo base_url() ?>js/jquery-ui-1.8.21.custom.min.js" type="text/javascript"></script>
            ...
            if (chks[i].checked){
               $('.checkquizzes:checked').each(function() {
                ...
                if ($('#qli'+$(this).val(),parent.document).length > 0) {
                 ...
                 var qmarks = parseInt($("span#queezmarks"+$(this).val()).html());
                  qmarks1+= qmarks;
                  //alert(qmarks);//queezmarks
                  $("#quizzestoaddlist",parent.document).append('<tr id="qli'+$(this).val()+'" style="cursor: move;" class="quizrow" ><td class="questIdd">'+$(this).val()+'</td><td>'+qname+'</td><td><span id = "totalM'+$(this).val()+'">'+qmarks+'</span></td><td><span class="removespan"><a href="javascript:void(0);" onclick="deleteRow(this, '+qmarks+')" class="removeele" id="remove'+$(this).val()+'">Remove</a></span></td><td hidden="hidden"><td hidden="hidden"><input type="hidden" name="qidck[]" id="qidck" value="'+$(this).val()+'"></td></tr>');
               ...
               //return true;
   				...
   				var tvalue = $("#totalmark",parent.document).val();
   				var qmarks2 = parseInt(qmarks1) + parseInt(tvalue);
   				//alert(qmarks1);						
				$("#totalmark",parent.document).val(qmarks2);
...
<div id="content">
<div id="content-top">
    <h2 class="que-list-style">Questions List</h2>
...
echo ($updType == 'create') ? form_open(base_url().'admin/quizzes/questlist', $attributes) : form_open(base_url().'admin/quizzes/editque/'.$question->id.'/'.$qid, $attributes);
//@@@@@@ changed the quizesaddlist to questlist in form action
?>


<div class="col-bcl-6 col-left">
<div id="table-3_length" class="dataTables_length">
<input placeholder="Question Tag" type="text" value="" name="search_text" class="form-control" style="width:auto;">
...
<a style="list-style: none; float: right;">
	<input type="button" class="btn btn-success" value="Save" id="addbut" name="addbut">
</a>
</div>
</div>
<div class="clr"></div>
...
echo ($updType == 'create') ? form_open(base_url().'admin/quizzes/quizesaddlist', $attributes) : form_open(base_url().'admin/quizzes/editque/'.$question->id.'/'.$qid, $attributes);
?>


<table class="table table-bordered responsive scroll" style="width: 100%;">
	<thead>
		<tr>
			<th width="5" style="color:#000;"><input type="checkbox" value="" name="toggle" onclick="checkAll(5)"></th>
			<th width="20" style="color:#000;width:7%">ID</th>
			<th style="color:#000;width:25.6%">Questions Tag</th>
			<th style="color:#000;width:20%">Questions Type</th>
			<th style="color:#000;width:11%">Questions Marks</th>
			<th style="color:#000;width:33%">Questions</th>			
		</tr>
	</thead>
...
<?php foreach ($questions as $quest):  ?>
	...
	<tr class="camp<?php echo $i;?>"> 	    
		<td>
        <input type="checkbox" title="Checkbox for row <?php echo $i;?>" value="<?php echo $quest->question_id?>" name="checkquizzes[]" id="checkquizzes[]" class="checkquizzes">
        </td>
	    <td><?php echo $quest->question_id;?></td>
	    <td><?php echo $quest->question_tag?></td>	    
	    <td><?php echo $quest->question_type;?></td>
	    <td style="width:14%"><span id="queezmarks<?php echo $quest->question_id?>"><?php echo $totalMarksOutOf;?></span></td>
		<td style="width:16%"><span id="queezname<?php echo $quest->question_id?>"><?php echo $quest->question;?></span></td>
	</tr>
<?php
endforeach ?>
