...
{	
	console.log('addquiz :'+ mediaid);
  parent.jQuery('#examinfo').load("<?php echo base_url();?>admin/quizzes/ajaxexamtotask2/"+mediaid+"/<?php echo $courseNo; ?>/<?php echo $sectionNo; ?>/<?php echo $proid; ?>");
	// parent.jQuery('#examinfo').load("<?php echo base_url();?>admin/quizzes/ajaxexamtotask2/"+mediaid+"/15");
  parent.jQuery('.examinfo').show();
  var sel_marks = jQuery(document).find('#Qmarks_'+mediaid).html();
  var sel_details =  jQuery(document).find('#Qdetails_'+mediaid).html();
  var sel_type =  jQuery(document).find('#Qtype_'+mediaid).html();
  parent.jQuery('#sel_marks').val(sel_marks);
  parent.jQuery('#sel_details').val(sel_details);
  parent.jQuery('#sel_type').val(sel_type);
  parent.jQuery('.box').attr('style','display:none!important');
	// window.parent.document.getElementById("db_media_15").value = mediaid;
  parent.jQuery('#db_media_15').val(mediaid);
...
<table class="table table-bordered responsive scroll" style="width: 100%;font-size:15px;">
	<thead>
		<tr>
			<th width="20" style="color:#000;font-weight:bold;width:1%">ID</th>
			<th style="color:#000;font-weight:bold;width: 48%">Quiz title</th>
			<th style="color:#000;font-weight:bold;width:23%">Questions</th>
			<th style="color:#000;font-weight:bold;width:10%">Marks</th>
      <th style="color:#000;font-weight:bold;width: 52%">Quiz Type</th>
      <th style="color:#000;font-weight:bold;width: 48%">Assigned to Course</th>
		</tr>
	</thead>
...
<?php foreach ($quizzes as $quiz){
	    // $ques =  $this->exam_model->get_count_ques($quiz->exam_id);
	    // print_r($ques);
	$j++;
//if ($quiz != NULL && is_object($quiz)) {
   ...
   // }                              ?>
	...
	<tr class="camp<?php echo $i;?>"> 	    
	    ...
	    <td style="width:38%;"><a href="javascript:void(0);" onclick="javascript:addquiz(<?php echo $quiz->exam_id?>);"><?php echo $quiz->exam_title; ?></a></td>		
	   <?php  
	                 // $quiz_quesarr = $this->exam_model->get_count_ques($quiz->exam_id); 

	    	$ques =  $this->exam_model->get_count_ques($quiz->exam_id);
	    	$sec =  $this->exam_model->get_count_sec($quiz->exam_id);
	    	$pg =  $this->exam_model->get_count_page($quiz->exam_id);
	    	// echo $quiz_quesarr;
	    	//  print_r($quiz_quesarr); 
	    	...
	    	// }
	    ?>
	    <td id="Qdetails_<?php echo $quiz->exam_id ?>" style="width:20%">
	    	<?php 
	    	      echo "<br>Section : ".$sec; 
	    	      echo "<br>Question : ". $ques->Qcount; ?>	    	      
	    </td>
      <td id="Qmarks_<?php echo $quiz->exam_id ?>" style="width: 10%"><?php echo $quiz->total_marks; ?></td>
      <td id="Qtype_<?php echo $quiz->exam_id ?>" style="width: 10%"><?php 
      switch ($quiz->exam_type) {
         ...
         <td >
            ...
            <?php $assign = $this->exam_model->getAssignCourse($quiz->exam_id); 
            // print_r($assign);
            if($assign)
...
<?php else: ?>
	...
	<div class='text'><?=lang('web_no_elements');?></div>
