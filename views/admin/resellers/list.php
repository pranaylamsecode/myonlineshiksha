		...
		<div id="toolbar" class="toolbar-list">
			...
			<ul id="sticky" style="list-style:none; float: right;  display: flex;">
				<!-- <li id="toolbar-new" class="listbutton" style="float: left; margin-right: 10px;">
					<a href="<?php echo base_url(); ?>admin/resellers/trashresellers/" class="btn btn-dark-grey">
						<span class="icon-32-new"></span><i class="entypo entypo-trash"></i>Trash
					</a>
				</li> -->
				<li id="toolbar-new" class="listbutton">
					<a href="<?php echo base_url(); ?>admin/resellers/create/" onclick="Joomla.submitbutton('edit')" class="btn">
...
<?php $i=0;
	$iii = 0;
		...
		$user_group_name = $this->Crud_model->get_single('mlms_groups',"id = ".$user->group_id,"title");
?> 
	<tr class="odd camp<?php echo $i;?>">
        ...
        <td class="field-title"><center><?php if(!empty($user->mobile)){ echo $user->mobile;} else{echo "-";}?></center></td>
        <td class="field-title"><?php echo $user->referral_code;?></td>
		<td class="field-title"><?php echo (!empty($user_group_name->title)) ? $user_group_name->title : '';?></td>
		<td>
		<?php if($user->active){ ?>
			<a title='Inactive referral' onclick="return change_status(<?php echo $user->id.",".$user->active;?>)" type="button"><div class='sprite 9999published' style="background-position: -340px 0;"></div></a>
			<?php }else{?>
			<a title="Active referral" onclick="return change_status(<?php echo $user->id.",".$user->active;?>)" type="button"><div class='sprite 999publish' style=" background-position: -308px 0;"></div></a>
			<?php }?>
		</td>
		<td class="editdelete">
        <?php
	...
	{
	?>
			<a class='' href='<?php echo base_url(); ?>admin/resellers/edit/<?php echo $user->id?>'><div class='sprite 2edit' style="background-position: -32px 0;" title="Edit User"></div></a>
	<?php
	}
	if(($u_data['groupid']=='4')  || ($maccessarr['users']=='own'))
	{
		...
		{
	?>
        <a class="" onClick="return deleteconfirm(<?php echo $user->id?>,'<?php echo $this->uri->segment(3)?>')" ><div class='sprite 4delete' style="background-position: -92px 0; width: 18px;" title="Delete"></div></a>
		<?php 
    	if($user->emailsent !=1 || $user->active !=1)
     		{ 
     	?>
     	<a class="" id="resendEmail_<?php echo $user->id?>" onClick="resendEmail(<?php echo $user->id?>)" ><i class="entypo entypo-cw" style="color: #888888;font-size: 25px;" title="Resend activation email"></i></a>
	<?php
			}
		}
		else
		{
     ?>
     	<a class="" onClick="return deleteconfirm2(<?php echo $user->id?>)" ><div class='sprite 4delete' style="background-position: -92px 0; width: 18px;" title="Delete"></div></a>
     	<?php 
	    	if($user->emailsent !=1 || $user->active !=1)
	     	{ 
     	?>
	...
	<tr>
		<td colspan="7">
			<p class='text'>No user available yet!</p>
		</td>
    </tr>
	<?php endif ?>    
</tbody>
		...
		confirm: function(){ 
				$.ajax({
	 				...
	 				success:function(returndata)
	 				{
	 					...
	 					var str = '<div class="alert alert-success alert-dismissible fade in"> <a href="#" class="close" data-dismiss="alert" aria-label="close"> <i class="fa fa-times" aria-hidden="true" style="margin-top:10px !important;"></i> </a><strong class="fa fa-check" aria-hidden="true"> </strong> <span style="margin-right:10px !important;">'+returndata+' &nbsp;</span></div>';
...
function resendEmail(id)
{
	$.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>admin/resellers/emailsent",
			...
			success: function(data)
			{
				console.log(data);
					if(data=="success")
					{
						...
						$.alert({
                           title: 'Sorry!',
                  		   content: '<p style="font-size: large;font-weight: bold;text-align: center;padding-top: 12px;">Email sending fail</p>',
                          confirm: function()
                                   {	
                                   		$("#resendEmail_"+id).attr('onclick','');
