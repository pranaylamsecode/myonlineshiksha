...
<div id="sticky" class="">
	<h2 class="Optional_quiz" style="display: inline-block;margin: 8px 20px !important;"><b>Total Marks : <span id="mkspan"> <?php echo $quiz->total_marks ? $quiz->total_marks : '0'; ?></span><input style="width: 100px;display: none" value="<?php echo $quiz->total_marks ? $quiz->total_marks : '0'; ?>" name="total_marks" class='totmk' >  <?php //echo $quiz->total_marks ? $quiz->total_marks : '0'; ?> </b>
...
<?php if ($qid != "0"): ?>
    ...
    <?php endif ?>
    	<a style="float: right; margin-right:10px;" class="btn btn-danger" id="btn_update" onclick="return validate()">Update</a>
...
<!-- tooltip area finish -->
					...
					<div class="form-group form-border" style="margin:0;">
						<label class="col-sm-12 field-title control-label">Department <span style="color:#FF0000;" class="error">*</span></label>
						<?php $CI = & get_instance();
						$CI->load->model('admin/pcategories_model'); ?>
						<div class="col-sm-12">
							
                           
                            <select name='department' id='department' class="form-control form-height">
                            <option value="0" <?php echo (@$quiz->department == $dept->id) ? 'selected="selected"' : '' ?>>None</option>
                        <?php

                    	   	$Deptcategories = $CI->pcategories_model->getDepartment();
                    		foreach($Deptcategories as $dept) { ?>
                    		 <option value="<?php echo $dept->id;?>" <?php echo (@$quiz->department == $dept->id) ? 'selected="selected"' : '' ?>><?php echo $dept->name; ?></option>
                    	<?php
                    		}
                         ?>
...
<!-- tooltip area finish -->
  ...
  <label>Compulsory Attempt Questions</label>
                        
                        <div class="col-sm-8">
                        <input class="" name="compulsory_que" id="compulsory_que" type="number" min="0" max="100" value="<?php echo $quiz->compulsory_que ? $quiz->compulsory_que : ''; ?>" style="width: 100px;">
                        <span id="err_pass_score" style="color: red;padding-left: 10px;font-size: 15px;"></span>
                    </div>
 ...
 -->                    
 <?php $date = new DateTime("now", new DateTimeZone('Asia/Calcutta') );
	$curr_date = $date->format('Y-m-d');
	$time = $date->format('H:i') ?>              
 						<div class="row">          
                         	<label Style="width:auto">Quiz Start On: <span class="required">*</span></label>

                                    <div class="col-sm-4" style="text-align: center;padding: 0px;margin: 0px;float: left;">

			                            <input type="date(Y-m-d)" class="ex_time" id="ex_start_date" name="ex_start_date" 
			                            value="<?php echo ((isset($quiz->ex_start_date)) ? $quiz->ex_start_date : $curr_date); ?> ">
			                            Date
			                        </div>
			                        <div class="col-sm-3" style="text-align: center;padding: 0px;margin: 0px;float: left;">
			                            <input type="time(H:i)" min="00:00:00" max="24:00:00"  class="ex_time" id="ex_start_time" name="ex_start_time"
			                            value="<?php echo ((isset($quiz->ex_start_time)) ? $quiz->ex_start_time : $time); ?> ">
			                            Time
			                        </div>
			                    </div>
			                        <div class="row">
				                       <label Style="width:auto">Quiz End On: <span class="required">*</span></label>

                                    <div class="col-sm-4" style="text-align: center;padding: 0px;margin: 0px;float: left;">

			                            <input type="date(Y-m-d)" class="ex_time" id="ex_end_date" name="ex_end_date"
			                            value="<?php echo ((isset($quiz->ex_end_date)) ? $quiz->ex_end_date : $curr_date); ?> ">
			                            Date
			                        </div>
			                        <div class="col-sm-3" style="text-align: center;padding: 0px;margin: 0px;float: left;">
			                            <input type="time(H:i)" min="00:00:00" max="24:00:00"  class="ex_time" id="ex_end_time" name="ex_end_time"
			                            value="<?php echo ((isset($quiz->ex_end_time)) ? $quiz->ex_end_time : $time); ?> ">
			                            Time
			                        </div>
			                        </div>
			                        <div class="row">
			                         <label Style="width:auto">Duration: <span class="required">*</span></label>

                                    <div class="col-sm-4" style="text-align: center;padding: 0px;margin: 0px;float: left;">
                                    	<input name="duration_h" id="duration_h" hours-only="" type="text" style="width: 50px;text-align: center" value="<?php echo ((isset($quiz->duration_h)) ? $quiz->duration_h : ''); ?> " placeholder="00" ><br>
			                            Hours
			                        </div>
			                        <div class="col-sm-3" style="text-align: center;padding: 0px;margin: 0px;float: left;">
			                            <input name="duration_m" id="duration_m" minutes-only="" type="text" style="width: 50px;text-align: center" value="<?php echo ((isset($quiz->duration_m)) ? $quiz->duration_m : ''); ?> " placeholder="00"><br>
			                            Minutes
			                        </div>
			                    </div>
                            </div>
  ...
  $sec_no = 1; $que_no = 0; 
  	...
  	foreach($page_list as $pg)
  	 { $i++;  ?>
		...
		<span id="page_<?php echo $i; ?>" class="openNav" ><img src="/public/images/page.07cfb8cf.png" role="presentation" data-radium="true" style="display: block;"></span>
...
function validate(){
	...
	$("#btn_update").html('wait... <i class="fa fa-spinner fa-spin fa-1x fa-fw"></i>');
	var ex_desc = tinymce.get('description').getContent();
	$(document).find('#ex_desc').text(ex_desc);
	// console.log(ex_desc);
	$("#submit").click();
...
<script>
	...
	$(document).ready( function() {
		...
		if(exam_type==3){
			...
			$('.Optional_quiz').show();
			// $('form#examsubmitid').find('#pass_score').val("");
		}
	});
...
$(document).on('change', '.ex_time', function(){
var startTime = $('#ex_start_time').val();
    var endTime = $('#ex_end_time').val();
    var sdate = $('#ex_start_date').val();
    var edate = $('#ex_end_date').val();

    date1 = new Date( sdate + ' '+startTime);

         date2 = new Date( edate+ ' '+endTime );
         ...
         $("#duration_h").val(hours);             
         // get minutes
         var minutes = Math.floor(res / 60) % 60; 
         $("#duration_m").val(minutes);    
         // get seconds
         var seconds = res % 60;
});
