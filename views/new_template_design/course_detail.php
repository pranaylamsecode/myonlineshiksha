...
$urlCourse = strtolower($programs->name);     
      ...
      $buy_links = "";
       ...
       <a class=" btn btn-lg btn-primary btn-block buy_course_btn" type="button" <?php echo $buy_links; ?>>
          <i class="entypo-book" style="color:#FFFFFF; margin-right:10px;"></i ><?php /*if(intval($programs->fixedrate) > 0) { */ echo $btn_msg;/*} else { echo "Enroll Now";} */?></a>
          <?php if($user_id)
          {
  ...
  <div class="modal fade" id="payment_modal" role="dialog" style="background-color: rgba(0,0,0,0.5);">
    <div class="modal-dialog" style="top: 10% !important">
      ...
      ?></span>
          ...
          <!-- <div class="paytext paytext_mb" style="padding-right: 5%;float: right">
            <span style="font-weight: 900;font-size: 20px;color: #777;">Buy using Wallet @ <?php echo 'Ai '.number_format($programs->aicash_rate);?></span>
            <input type="hidden" id="aicash_rate" value="<?php echo $programs->aicash_rate;?>">
          </div> -->
        </div>
        <div class="modal-body">
                ...
                <div class="tab-pane active" id="stripe" style="padding:10px 0; border:0; ">
                  ...
                  <div style="display: inline-block;width: 100%;">
                    <div class="col-xs-12 col-sm-12 form-group">
                      <h4>Batch <span style="padding-left: 5px;font-size: 13.5px">(select batch you want to enroll)</span></h4>
                      <select class="form-control" id="batch_id">
                        <option value="">-- select Batch --</option>
                        <?php foreach ($batches as $batch) { ?>
                        <option value="<?php echo $batch->id;?>"><?php echo ucwords($batch->batch_name).' &nbsp; ( '.date('h:i A',strtotime($batch->batch_start_time)).' - '.date('h:i A',strtotime($batch->batch_end_time)).' )'; ?></option>
                      <?php } ?>
                      </select>
                    </div>
                  </div>
...
$currency_code      = "INR";            
$total              = intval($programs->fixedrate) * 100; // 100 = 1 indian rupees
$amount             = $programs->fixedrate;
...
$(document).on('click', '#razorPay', function (e) {
  ...
  $(this).html('<i class="fa fa-spinner fa-pulse fa-1x fa-fw"></i><span style="padding-left:10px;">Please wait...</span>');
    ...
    $.ajax({
        type : 'post',
        cache : false,
        url : '<?php echo base_url();?>paymentprocess/create_order/',
        data : {user_id: user_id, course_id: course_id, batch_id: batch_id, promocode: promocode,valid_coupon: valid_coupon,merchant_amount : merchant_amount,merchant_order_id :merchant_order_id},
